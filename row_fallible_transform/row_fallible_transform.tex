...
\title{Privacy Proofs for OpenDP: Row Fallible Transform}
\author{Grace Tian}
\date{Summer 2021}
\begin{document}
...
The current OpenDP library contains the \texttt{make\_row\_by\_row\_fallible} function implementing the row transform function. This is defined in lines 29-46 of the file \texttt{manipulation.rs} in the Git repository (\url{https://github.com/opendp/opendp/blob/main/rust/opendp/src/trans/manipulation.rs#L29-L46}).

\includegraphics[width=\textwidth]{make_row_by_row_fallible.jpg}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Type \texttt{F} has trait \texttt{Fn(\&DIA::Carrier) -> Fallible(DOA::Carrier)} \grace{Ask Mike about this.}
    \end{itemize}
\end{itemize}
...
def make_row_by_row_fallible(atom_input_domain : DIA, atom_output_domain : DOA, atom_function : F):
    ...
    # how do I incorporate "fallible" for function call?
...
\begin{theorem}
    For every setting of the input parameters \texttt{(atom\_input\_domain, atom\_output\_domain, atom\_function)} to \texttt{make\_row\_by\_row\_fallible} such that the given preconditions
    hold, the transformation returned by \texttt{make\_row\_by\_row\_fallible} has the following properties:
    \begin{enumerate}
...
\begin{proof}
\begin{enumerate}
\item \textbf{(Appropriate output domain).} In the case of \texttt{make\_row\_by\_row\_fallible}, this corresponds to showing that for every vector $v$ of elements of type \texttt{DIA}, \texttt{function}$(v)$ is a vector of elements of type \texttt{DOA}. 
