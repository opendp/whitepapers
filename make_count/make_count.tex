...
\notebig{This proof document incorporates all suggestions made as of Sept. 4, 2021. The only ``todos'' that remain are those that concern inserting versioned links to the definitions documents. These documents will hopefully be pushed within the next two days, after which I will add the versioned links. There is also one question about the proof of lemma \ref{lemma:len-sum-equiv} (this lemma is new); the question appears immediately after the proof of the lemma.}

\section{MakeCount}

\subsection{Version of definitions docs}

This document uses the version of the definitions docs published as of September 3, 2021. These documents are available at

\todo{Insert URL to version of definitions docs.}
...
% From definition \ref{defn:symm-dist}, we know that $h_v(z)$ will return the number of occurrences of value $z$ in vector $v$. 

% Because every element in the vector will be summed over, and because the result for each element is equal to the number of occurrences of that element in the vector, the result will be the number of occurrences 
