...
\silvia{Decide one of the following 2:}
...
Given a vector domain \texttt{VectorDomain(D)}, the \textit{symmetric difference} between any two vectors $u, v \in \texttt{VectorDomain(D)}$, denoted $\MultiSet(u)$ $\Delta \MultiSet(v)$, corresponds to the multiset representation of elements which are in either $u$ or $v$ but not in their intersection.
...
Given an arbitrary vector domain \texttt{VectorDomain(D)}, %or \texttt{SizedDomain(VectorDomain(D))}, 
the \textit{symmetric distance} between any two vectors $u, v \in$ \texttt{VectorDomain(D)}, %or $\in$ \texttt{SizedDomain(VectorDomain(D))}, 
...
\begin{definition}[Absolute distance]\label{def:abs}
    Given two elements $n, m$ in \texttt{AllDomain(T)}, where \texttt{T} denotes an arbitrary type with trait \texttt{Sub(Output=T)}, the absolute distance between $n$ and $m$, denoted $d_{Abs}$, is defined as $d_{Abs}(n, m)= |n-m|$, where the horizontal bars represent absolute value. Absolute distance has associated generic type \texttt{Q}.
\end{definition}

\todo{What data types is this operation defined on?} \silvia{I think any type \texttt{T} with trait \texttt{Sub(Output=T)}.}
...
\questionc{Should we add (or move) this to the \href{https://www.overleaf.com/project/60d215bf90b337ac02200a99}{``List of definitions used in the pseudocode''} doc?} \silvia{If we do it, we should move all of the metrics. I like it more here, but that is also from when we were using preconditions, because then the metric names never appear in the pseudocode.}

\begin{claim}
    Absolute distance is a metric.
\end{claim}

Symmetric distance does not have an associated generic type; it is hard-coded to \texttt{u32}. It is the only such metric in the library that is hard-coded to an specific type. For example, absolute distance has the associated generic type \texttt{Q}, as stated in Definition~\ref{def:abs}

This is why we write \texttt{metric = SymmetricDistance()}, which implicitly implies that the associated $\din, \dout$ has type \texttt{u32}, but \texttt{metric = AbsoluteValue(T)}, which implies that the associated $\din, \dout $ has type \texttt{T}. Therefore, per our pseudocode structure, all the type signatures follow directly from the domain and metrics specified.

\subsection{Distances}
\begin{definition}[$k$-close]
For any metric $d$, we say that two elements $u, v$ are $k$-close under $d$ if $d(u, v) \leq k$. \silvia{Needs fixing after July 13 meeting: $\din, \dout$ is more general than metrics; e.g., $(\epsilon, \delta)$-DP.}
\end{definition}
